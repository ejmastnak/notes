...
import vibeRaw1Img from './img/vibe-test/vibe-1-raw.jpg?w=800'
import vibeFilt1Img from './img/vibe-test/vibe-1-filt.jpg?w=800'
import vibeRaw2Img from './img/vibe-test/vibe-2-raw.jpg?w=800'
import vibeFilt2Img from './img/vibe-test/vibe-2-filt.jpg?w=800'

import noVibeFilt1Img from './img/vibe-test/no-vibe-1-filt.jpg?w=800'
import noVibeFilt2Img from './img/vibe-test/no-vibe-2-filt.jpg?w=800'
import noVibeRaw1Img from './img/vibe-test/no-vibe-1-raw.jpg?w=800'
import noVibeRaw2Img from './img/vibe-test/no-vibe-2-raw.jpg?w=800'
...
However, the haptic motor does seem to cause a steady creep of the MC signal (again, this is from initial testing, and more data is needed).
...
- With MC sensor recording, haptic motor iterates through waveforms in the DRV2605L waveform library (see page 60 of DRV2605L datasheet); the idea is to use many different waveforms to get as much data as possible on the effect of vibration on MC signal acquisition.
- With MC sensor recording, contract/release vastus lateralis contracts/relaxes at cca 80 bpm, measured with a metronome, (80 bpm parallels target cycling cadence) for 5-10 repetitions.
...
### MC sensor with vibration 1

Here is a sample measurement with vibration on, first the raw signal and then filtered.

<Image :src="vibeRaw1Img" caption="Raw MC sensor signal with haptic motor vibration on, using testing protocol described above. Note the creep of the MC signal." />
<Image :src="vibeFilt1Img" caption="The above signal, filtered. The noise from the motor's vibration disappears." />

### MC sensor with vibration 2

Here is a second measurement with vibration on, first the raw signal and then filtered.
The vastus lateralis is contracting for the first 8 seconds, and resting for the next 8 seconds. Note how the creep continues throughout, even when the vastus lateralis is not contracting.

<Image :src="vibeRaw2Img" caption="Raw MC sensor signal with haptic motor vibration on, using testing protocol described above. Note the creep of the MC signal." />

<Image :src="vibeFilt2Img" caption="The above signal, filtered. The noise from the motor's vibration disappears." />

### MC sensor without vibration 1

Here is a sample measurement with vibration *off*, first the raw signal and then filtered. Note how there is no creep.

<Image :src="noVibeRaw1Img" caption="Raw MC sensor signal without haptic motor vibration on, using testing protocol described above.  There is no creep of the MC signal. The large peak is a tap of the MC sensor with a finger." />
<Image :src="noVibeFilt1Img" caption="The above signal, filtered." />

### MC sensor without vibration 2

Here is a second measurement with vibration off, first the raw signal and then filtered.

<Image :src="noVibeRaw2Img" caption="Raw MC sensor signal without haptic motor vibration on, using testing protocol described above.  There is no creep of the MC signal." />
<Image :src="noVibeFilt2Img" caption="The above signal, filtered." />
